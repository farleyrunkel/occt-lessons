...
int main() {
    ...
    TDF_Tool::TagList(cylinderLabel, intergers);
    ...
    TDF_Tool::TagList(resultLabel, intergers);
    ...
    while (faceExplorer.More()) {
        ...
        TDF_Tool::TagList(faceLabel, intergers);
        faceExplorer.Next();
    }
    ...
    if (firstFaceExplorer.More()) {
        ...
        TDF_Tool::TagList(selectedLabel, intergers);
    }
    ...
    cutOp = BRepAlgoAPI_Cut(modifiedBox, cylinder); // 重新执行切割
    TopoDS_Shape newResult = cutOp.Shape();
    TNaming_Builder newResultBuilder(resultLabel);
    newResultBuilder.Modify(result, newResult); // 更新结果的命名形状

    TDF_LabelMap     valid;
    valid.Add(selectedLabel);
    // 8. 重新计算选定面
    if (!selector.Solve(valid)) {
        std::cout << "Failed to resolve selected face after modification." << std::endl;
    } else {
        Handle(TNaming_NamedShape) namedShape = selector.NamedShape();
        TopoDS_Shape resolvedFace = TNaming_Tool::GetShape(namedShape);
        std::cout << "Resolved selected face after modification: " << resolvedFace.IsNull() << std::endl;
    }

    // 9. 保存文档（可选）
    app->SaveAs(doc, "result.cbf");
